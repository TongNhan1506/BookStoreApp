...
public class ImportPanel extends JPanel implements Refreshable {
    ...
    private void createTicket() {
        ...
        int supplierId = 0;
        try {
            supplierId = Integer.parseInt(cboTicketSupplier.getSelectedItem().toString().split(" - ")[0]);
        } catch (Exception ex) {}

        if (supplierId == 0) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn Nhà cung cấp thực tế ở góc trên bên phải!");
            return;
        }
